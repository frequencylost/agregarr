  ...
  anilistCustomListUrl: Yup.string().when(['type', 'subtype'], {
    ...
    then: (schema) =>
      schema
        ...
        .matches(
          /anilist\.co\/(?:user\/[^/]+\/(?:animelist|list)\/[^/?]+|(?:animelist|list)\/[^/?]+|search\/anime(?:\/[^/?]+)?|anime\/?\d+|staff\/\d+|studio\/\d+)/,
          'Please enter a valid AniList URL (e.g., user lists, search pages, anime pages, staff pages, or studio pages)'
        ),
    otherwise: (schema) => schema,
  }),
